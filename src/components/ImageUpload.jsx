...
export default function ImageUpload({ images, onAdd, onRemove, onTranscribe, onTranscribeAll }) {
  ...
  return (
    <div className="card space-y-5">
      <h2 className="section-title mb-0">Upload Race Sheets</h2>
      ...
      >
        ...
        <p className="text-slate-400 text-sm mt-1">
          JPG, PNG, HEIC — multiple files accepted
        </p>
        {images.some((i) => i.file?.size > 4 * 1024 * 1024) && (
          <p className="text-amber-600 text-xs mt-2">
            Large images detected — keep under 4 MB for best results
          </p>
        )}
      </div>
  ...
  return (
    <div className="relative group rounded-xl overflow-hidden border border-slate-200 bg-white shadow-sm flex flex-col">
        ...
        {img.status !== 'processing' && (
          <button
            ...
            className="absolute top-2 right-2 w-7 h-7 bg-black/50 text-white rounded-full flex items-center justify-center
                       opacity-0 group-hover:opacity-100 focus:opacity-100 transition hover:bg-black/75 text-base leading-none"
          >
            ×
          </button>
        )}
      ...
      <div className="p-2 space-y-1.5 flex-1">
        <p className="text-xs text-slate-600 truncate font-medium" title={img.name}>
          {img.name}
        </p>
        <span
          className={`inline-block text-xs px-2 py-0.5 rounded-full font-semibold ${STATUS_STYLE[img.status]}`}
        >
